...
include_once('JamefClass.php');

class FKcorreiosg2cp4FreteClass {
    ...
    public function calculaFreteSimulador($origem, $dadosBasicos, $params) {
        ...
        $sql = "SELECT
                    "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras.*,
                    "._DB_PREFIX_."carrier.id_reference
                FROM "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras
                    INNER JOIN "._DB_PREFIX_."carrier
                        ON "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras.id_carrier = "._DB_PREFIX_."carrier.id_carrier
                WHERE "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras.ativo = 1 AND
                      "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras.id_shop = ".(int)$this->context->shop->id;
        ...
        foreach ($transportadoras as $transp) {
            ...
            foreach ($regioes as $regiao) {
                ...
                if ($origem == 'produto') {
                    // Calcula cubagem em mts
                    $cubagem = ($params['product']->height / 100) * ($params['product']->width / 100) * ($params['product']->depth / 100);
                    ...
                    foreach ($this->context->cart->getProducts() as $prod) {
                        ...
                        // Calcula cubagem
                        $cubagem = ($prod['height'] / 100) * ($prod['width'] / 100) * ($prod['depth'] / 100);
                ...
                $codigoFilial = $regiao['codigo_filial'];
                $ufFilial = $regiao['uf_filial'];
        ...
        $sql = "SELECT
                ...
                FROM "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras
                    INNER JOIN "._DB_PREFIX_."carrier
                        ON "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras.id_carrier = "._DB_PREFIX_."carrier.id_carrier
                WHERE "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras.ativo = 1 AND
                      "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras.id_carrier = ".(int)$idCarrier." AND
                      "._DB_PREFIX_."fkcorreiosg2cp4_transportadoras.id_shop = ".(int)$this->context->shop->id;
    ...
    private function calculaValor($parm) {
        ...
        if ($cache['status']) {
            ...
            // Instancia JamefClass
            $jamefClass = new JamefClass();

            $jamefClass->setCnpj($parm['cnpj']);
            $jamefClass->setCepDestino($parm['cepDestino']);
            $jamefClass->setValorPedido($valorPedidoBaseFrete);
            $jamefClass->setPesoPedido($pesoPedidoBaseFrete);
            $jamefClass->setCubagemPedido($totalCubagem);
            $jamefClass->setcodigoFilial($parm['codigoFilial']);
            $jamefClass->setufFilial($parm['ufFilial']);

            if ($jamefClass->calculaPreco()) {

                // Recupera dados retornados pelos Correios
                $totalFrete = $jamefClass->getValorFrete();
    ...
    private function criaHash($idTransp, $cnpj, $codFilial, $ufFilial, $cepDestino, $cubagem, $valorPedido, $pesoPedido) {
    ...
    private function gravaCache($hash, $valorFrete) {
        ...
        Db::getInstance()->insert('fkcorreiosg2cp4_cache', $dados);
    ...
    private function recuperaCache($hash) {
        ...
        $sql = "SELECT *
                FROM "._DB_PREFIX_."fkcorreiosg2cp4_cache
                WHERE hash = '".$hash."'";
